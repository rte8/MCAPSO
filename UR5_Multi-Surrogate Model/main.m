...
%关节角度限制，因为要与18维的个体匹配，所以六个关节角度限制写成3组一样的，共18个
...
if (clientID>-1)
    ...
    % 产生预设轨迹的三个点坐标
    goal_position = [0.2618,-0.3958,0.7193,-0.3215,-0.2154,0.8642,-0.6021,0.3895,0.603];  
    ...
    while i < maxgen
        i = i + 1;
        w = ws - (ws - we)*i/maxgen + (0.5 - rand)*(1-i/maxgen)^2;  
        flag = 0;  
        
        if i == traingen
            [net_cnn,ps_input] = TrainCnn(AP);  % 收集n代数据后训练代理模型
            [net_cnn1,ps_input1] = TrainCnn1(AP);
        end
        ...
        for j = 1:sizepop
            ...
            if flag == 1
                ...
                [r3, ~, retFloats1, ~, ~] = sim.simxCallScriptFunction(clientID, 'UR5', sim.sim_scripttype_childscript, 'sendTableToMatlab1', [], [], [], '', sim.simx_opmode_blocking);
                if r2 == sim.simx_return_ok && r3 == sim.simx_return_ok
                %  1代表碰到了，2代表没碰到
                    min_distance = min(retFloats);
                    min_distance1 = min(retFloats1);
                    if min_distance == 0
                        collision_flag = 1;
                    else
                        collision_flag = 2;
                    ...
                    min_distance = min(min_distance,min_distance1);
                end
                % 记录每次迭代后的全部种群及其对应的轨迹中的最短距离和轨迹是否碰撞标志
                apop(j,:) = [pop(j,:),min_distance,collision_flag,collision_flag1];
                AP = [AP;apop(j,:)];
            end
        ...
        if flag == 1 && i >= traingen
            accuracy = TestCnn(net_cnn,apop,ps_input);  % 测试代理模型精度
            accuracy1 = TestCnn1(net_cnn1,apop,ps_input1);
            accuracy = 0.5*accuracy + 0.5*accuracy1;
        ...
        % 代理评价
        if flag == 0
            apop = TestCnn_1(net_cnn,pop,ps_input,net_cnn1,ps_input1);
        ...
        [y1,~] = testfun(zbest,goal_position);
        if y1 <= 0.025
           break;
        end
